...
- (勘定科目)
    ...
    - $profit := $#mkset("収益")
    - $loss := $#mkset("費用")
    - $accounting := assets union liabilities union netassets union profit union loss$
- (仕訳要素)$journalitem := accounting times NN$
//- (仕訳要素集合)$journalitems := {x in 2^journalitem | 0 < abs(x) < infinity}$
- (仕訳)$journalline := date times {v_1 = v_2 | ((d, v_1), (d, v_2)) in journalitem^2}$
...
- (複合仕訳)$journalmultiline := date times {0 < abs(I) < infinity and 0 < abs(J) < infinity and (abs(I) = 1 or abs(J) = 1) and sum_i v_i = sum_j v'_j | ({(d_i, v_i) | i in I}, {(c_j, v'_j) | j in J}) in (2^journalitem)^2}$
- (複合仕訳の分解)$forall ({(d_i, v_i) | i in I}, {(c_j, v'_j) | j in J}) in journalmultiline. exists {((d''_k, v''_k), (c''_k, v''_k)) | k in K} in {J in 2^journalline | abs(J) < infinity}. (forall i in I. sum_i' 1_(c_(i') = c_i) v_i = sum_k 1_(c''_k = c_i) v''_k and forall j in J. sum_j' 1_(c_(j') = c_j) v'_j = sum_k 1_(c''_k = c_j) v''_k)$
- (仕訳帳)$journal := {abs(J) < infinity | J in 2^journalmultiline}$
